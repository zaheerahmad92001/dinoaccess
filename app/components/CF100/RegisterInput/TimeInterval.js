...
class TimeInterval extends Component {
    ...
    constructor(props) {
        ...
        this.registerNumbers = registerNames.map(name => CF100[name].register)
    ...
    save() {
        let { register, registers } = this.props
        let registerNames = [register.register, ...register.others]
        let registerNumbers = registerNames.map(name => CF100[name].register)
        ...
        return (
            <View style={{ height: '100%', padding: 20, paddingTop: 150, display: 'flex', alignItems: 'center' }}>
                <Checkbox
                    ...
                    label={translate('cf100.setpoints.' + (this.state.enabled != 'yes' ? 'deactivated' : 'activated'))}
                    labelStyle={{ marginLeft: 5, fontSize: 16 }}
                    style={{ marginTop: 10, marginBottom: 30 }}
                />
                
                <View style={{ opacity: this.state.enabled == 'yes' ? 1 : 0.6 }}>
                    <Label>{ translate('cf100.setpoints.on') }</Label>
                    ...
                    <Label>{ translate('cf100.setpoints.off') }</Label>
                ...
                <Toast ref="toast" />
                <PillButton style={{ backgroundColor: 'rgba(255, 255, 255, 0.25)', marginTop: 40, opacity: changed ? 1 : 0.5 }} textStyle={{ color: 'white' }} title={ translate('cf100.save') } onPress={() => this.save()} disabled={!changed} />
            </View>
        )
    }
