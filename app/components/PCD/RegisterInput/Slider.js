...
class Slider extends Component {
    ...
    increase() {
        ...
        let value = Math.min(register.range[1], this.state.value + 1)
        this.setState({ value })
    }
    ...
    decrease() {
        ...
        let value = Math.max(register.range[0], this.state.value - 1)
        this.setState({ value })
    }
    ...
    render() {
        ...
        let { register: number, range, unit, comma, label } = register
        ...
        return (
            <View style={{ height: '100%', padding: 20, paddingTop: 52, display: 'flex', alignItems: 'center' }}>
                ...
                <View style={{ position: 'absolute', bottom: 30, width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <PillButton style={{ backgroundColor: 'rgba(255, 255, 255, 0.25)', opacity: changed ? 1 : 0.5 }} textStyle={{ color: 'white' }} title={ translate('pcd.save') } onPress={() => this.save()} disabled={!changed} />
                </View>
            </View>
        )
    }
