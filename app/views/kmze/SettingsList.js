...
class SettingsList extends Component {
    ...
    async searchFirmwareUpdate() {
        try {
            ...
            async function searchForUpdate(type, version) {
                let { data } = await axios.get('firmware/list', {
                    params: {
                        type, version,
                    }
                })
            ...
            let deviceUpdate = await searchForUpdate(device.type, registers['KMZE_fw'])
            // KMZE basiert nur noch auf einem ESP. -> Nur ein Update notwendig
            //let espUpdate = await searchForUpdate(`ESP_${device.type}`, registers['ESP_fw'])

            if (deviceUpdate) {
                this.setState({ modal: 'firmware', deviceUpdate })
            } else {
    ...
    render() {
        ...
        let deviceSettings = [
            ...
            { title: translate('settings.serial'), value: '' + registers.KMZE_id, readonly: true },
            { title: translate('settings.firmware'), value: `${registers.KMZE_fw}`, readonly: true },
        ]
