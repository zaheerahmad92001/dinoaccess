...
class SettingsList extends Component {
    ...
    textInputModal(name, title, text = '', value = '', numeric = false, onSave) {
        return <TextInputModal
            isVisible={this.state.modal == name}
            keyboardInputType={ numeric ? 'numeric' : 'default' }
            title={title}
            description={text}
            value={value}
            onSave={onSave}
    ...
    saveFunction(field) {
        return async (value) => {
            ...
            if (field == 'code') {
                this.props.dispatch({
                    type: DeviceActions.SET_DEVICE_CODE,
                    token: this.props.token,
                    code: value
                })
            } else if (field == 'interval') {
                let saved = socket.saveRegister(this.props.token, 147, value)
            } else {
    ...
    async searchFirmwareUpdate() {
        try {
            ...
            let deviceUpdate = await searchForUpdate(device.type, registers[146])
    ...
    render() {
        ...
        let refreshInterval = registers[147] || 10000
        let intervals = {
        ...
        let code = this.props.enteredCode ? this.props.enteredCode.code : ''

        let deviceSettings = [
            { title: translate('settings.code'), value: code, onPress: () => this.setState({ modal: 'code' }) },
            { title: translate('settings.name'), value: device.display_name, onPress: () => this.setState({ modal: 'name' }) },
            { title: translate('settings.interval'), value: intervals[refreshInterval], onPress: () => this.setState({ modal: 'interval' }), readonly: !device.isOnline },
            { title: translate('settings.schema') },
            { title: translate('settings.serial'), value: '' + registers.PCD_id, readonly: true },
            { title: translate('settings.firmware'), value: `${registers[146]}`, readonly: true },
        ]
        ...
        return <ScrollView style={{ backgroundColor: '#FAFAFA', minHeight: '100%' }} contentContainerStyle={{ paddingTop: getStatusBarHeight() }}>
            {this.textInputModal('name', translate('settings.name'), '', device.display_name, false, this.saveFunction('name'))}
            {this.selectModal('interval', translate('settings.interval'), intervals, refreshInterval, '', this.saveFunction('interval'))}
            {this.textInputModal('address', translate('settings.city'), '', device.address.city, false, this.saveFunction('city'))}
            {this.textInputModal('zipcode', translate('settings.zip'), '', device.address.zip, false, this.saveFunction('zip'))}
            {this.textInputModal('code', translate('settings.code'), '', code, true, this.saveFunction('code'))}
